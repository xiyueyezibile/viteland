...
const routes$1 = [
  { path: "/guide/a/", element: MDXContent$3 },
  { path: "/guide/b/", element: MDXContent$2 },
  { path: "/guide/", element: MDXContent$1 },
  { path: "/", element: MDXContent }
];
...
const Content = () => {
  const routes$12 = routes.map((it) => {
    const reactElement = React.createElement(it.element);
    return { path: it.path, element: reactElement };
  });
  console.log(routes, routes$12);
  const routeElement = useRoutes(routes$12);
...
const data = { "title": "11", "description": "SSG Framework", "themeConfig": {}, "vite": {} };
const siteData = data;
function App() {
